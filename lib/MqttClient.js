...
const MqttClient = function(options) {
    this.brokerURL = process.env.MQTT_BROKER_URL || options.brokerURL;
    ...
    this.omitMapDataPixels = options.omitMapDataPixels || false;
...
MqttClient.prototype.connect = function() {
    if(!this.client || (this.client && this.client.connected === false && this.client.reconnecting === false)) {
        ...
        console.log("Connecting to MQTT Broker: " + this.brokerURL.split('@')[1]);
    ...
    if ((this.client && this.client.connected === true && (this.publishMapImage || this.publishMapData)) || this.webserverEnabled) {
        new Promise((resolve,reject) => {
            ...
            if (this.publishMapData && this.client && this.client.connected === true) {
                ...
                // Omit data pixels (floor, walls) if expected (huge size gain)
                if(this.omitMapDataPixels) delete parsedMapData.image.pixels;
                //
